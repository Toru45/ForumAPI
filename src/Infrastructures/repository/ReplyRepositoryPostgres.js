...
class ReplyRepositoryPostgres extends ReplyRepository {
  ...
  async getRepliesByCommentId(commentId) {
    const query = {
      text: 'SELECT replies.id, replies.comment, replies.owner, users.username, replies.date, replies.content, replies.is_delete FROM replies LEFT JOIN users ON users.id = replies.owner WHERE replies.comment = $1 ORDER BY replies.date ASC',
      values: [commentId],
    };
